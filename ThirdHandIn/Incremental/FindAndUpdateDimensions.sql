...
SET validTo = DATEADD(DAY, -1, SELECT(CurrentValid FROM OrderSaleProcess.dbo.Validator))
WHERE ProductID IN (
...
SELECT ProductID, ProductName, SupplierName,SELECT(CurrentValid FROM OrderSaleProcess.dbo.Validator),'12/31/9999'
FROM OrderSaleProcess.dbo.Product_changed
go 
...
--find the added rows
SELECT ProductID, ProductName, CompanyName,SELECT(CurrentValid FROM OrderSaleProcess.dbo.Validator),'12/31/9999'
FROM Products
JOIN Suppliers on Products.SupplierID = Suppliers.SupplierID
WHERE ProductID IN 
((
...
SELECT EmployeeID, LastName, FirstName,SELECT(CurrentValid FROM OrderSaleProcess.dbo.Validator),'12/31/9999'
FROM OrderSaleProcess.dbo.Employee_changed
go 
...
SELECT CustomerID, Country,SELECT(CurrentValid FROM OrderSaleProcess.dbo.Validator),'12/31/9999'
FROM OrderSaleProcess.dbo.Customer_changed
go 
